  ...
  describe('createImageName()', () => {
    test('returns the git remote', async () => {
      ;(utils.exec as jest.Mock).mockImplementation(async () => {
        return 'origin  git@github.com:microservices/oms.git (fetch)\norigin  git@github.com:microservices/oms.git (push)'
      })

      expect(await utils.createImageName()).toBe('oms/microservices/oms')
    })
    ...
    test('returns the git remote for UI', async () => {
      ...
      expect(await utils.createImageName(true)).toBe('microservices/oms')
    })
    ...
    test('returns the http', async () => {
      ...
      expect(await utils.createImageName()).toBe('oms/microservice/machinebox-classificationbox')
    })

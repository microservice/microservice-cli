...
import { OMGValidate, Microservice, Action, Command, Argument } from '@microservices/validate'
...
export default class Cli {
  ...
  public actionHelp(actionName: string): void {
    ...
    if (action.events) {
      utils.error(
        `The action \`${
          action.name
        }\` is an event action and must be called using \`oms subscribe\`. Try running \`oms subscribe ${
          action.name
        } --help\``,
      )
      process.exit(1)
  ...
  public eventActionHelp(actionName: string): void {
    ...
    this.helpForActionHeader(action, stringBuffer)
    stringBuffer.push(`\n    Events: (run in the form of \`oms subscribe ${action.name} \`event\`\`)\n`)
    const padding = '          '
  ...
  public async run(action: string, givenOptions: any): Promise<void> {
    ...
    if (!options.args || !options.envs) {
      utils.error('Failed to parse command, run `oms run --help` for more information.')
      process.exit(1)
      return
    }
    ...
    if (options.image) {
      if (!utils.doesContainerExist(options.image, await utils.docker.listImages())) {
        utils.error(`Image for microservice is not built. Run \`oms build\` to build the image.`)
        process.exit(1)
        return
      }
    } else {
    ...
    if (process.argv[2] === 'run' && this._run.constructor.name === 'EventRun') {
      utils.error(`Action \`${action}\` is and event. Use \`oms subscribe\``)
      process.exit(1)
    }
    let spinner
    if (!options.raw) {

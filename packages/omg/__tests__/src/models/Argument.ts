    ...
    test('throws an exception because the json is not valid', () => {
      try {
        ...
        expect(e).toEqual({
          ...
          issue: {},
          text: "actions.action.arguments.name should have required property 'type'",
          valid: false,
        })
      }
    })
    ...
    test('throws an exception because the json is not valid', () => {
      try {
        ...
        expect(e).toEqual({
          ...
          issue: { type: 'bob' },
          text:
            'actions.action.arguments.name.type should match pattern "^(number|int|float|string|uuid|list|map|boolean|path|object|any)$"',
          valid: false,
        })
      }
    })

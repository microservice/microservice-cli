...
describe('HttpRun.js', () => {
  ...
  beforeEach(() => {
    ...
    utilsDockerCreateContainer = sinon
      .stub(utils.docker, 'createContainer')
      .callsFake(async () => {
        return {
          start: () => {},
          $subject: {
  ...
  describe('.isRunning()', () => {
    ...
    beforeEach(() => {
      utilsDockerGetContainer = sinon
        .stub(utils.docker, 'getContainer')
        .callsFake(container => {
          return {
            inspect: async () => {
              return {
  ...
  describe('.exec(action)', () => {
    ...
    test('action that deletes', async () => {
      ...
      expect(
        rpDeleteStub.calledWith('http://localhost:5555/user/2')
      ).toBeTruthy()
    })
  })
})

...
echo "===== Testing d ====="
cd d
output="$(../../omg run --raw message -a name="Peter" | jq)"
...
echo "===== Testing hashes ====="
cd ../hashes
output="$(../../omg run --raw digest -a method="sha1" -a data="foo" | jq)"
if [ "$output" != '{"method":"sha1","digest":"0BEEC7B5EA3F0FDBC95D0DD47F3C5BC275DA8A33"}' ] ; then echo "$output"; exit 1; fi
output="$(../../omg run --raw hmac -a method="sha1" -a data="hello world" -a secret="my secret" | jq)"
if [ "$output" != '{"method":"sha1","digest":"9F60EE4B05E590A7F3FAC552BFB9D98FA46F78D9"}' ] ; then echo "$output"; exit 1; fi

# echo "===== Testing jwt ====="
# cd ../jwt
# output="$(../../omg run --raw sign -a data="hello world" -a secret="abc" -a expiresIn="2h" | jq)"
...
# output="$(../../omg run --raw verify -a token="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJkYXRhIjoiaGVsbG8gd29ybGQiLCJpYXQiOjE1NDM5MjMyMTAsImV4cCI6MTU0MzkzMDQxMH0.FCsstg1m01goffz0cFYxZIUe0uPybUAqzGRnZPJgGBw" | jq)"
